...
`SORT REL REFL TRANS SYMM TRANSPORT` should be filled respectively by the sort, 
the relation term, and its reflexivity, transitivity, symmetry, and transport 
rules. `_` can be used for a missing symmetry or transport member. The 
annotation is declarative. It may sit on any assertion, conventionally the 
reflexivity axiom. Bundle members must be in rule form (hypotheses with `>`, 
not object-level implications) with no bound binders. 
...
arguments map back to to the rule arguments; the rest are phantom, view-local 
slots. At most one `@view` per rule.
...
`RULE` is a rule name, already in scope, at most one per rule. If the annotated
rule's application fails the compiler retries the whole application with the 
named rule, recursively following chains. This is what lets a theory expose one 
user-facing name for a family of variant rules.

...
`x` itself. It appears nowhere in what the user writes, so it can't be 
recovered from any kind of walk of the user expressions. Luckily, it can be 
provided by an arbitrary choice.

This is what the `@fresh` annotation is for: `--| @fresh x` fills the binder 
from the sort's `@vars` pool (here the `z` registered at the top of the theory 
) before the view is matched. The `fol-rules` rules get away without it only 
because their bound binder is visible in the `∀` or `∃` of the line itself.

...
equalities or equivalences. For a general goal, the chain must reach an earlier 
line or hypothesis. For an equation, it may instead connect the equation's two 
sides. The search uses [e-graph](https://en.wikipedia.org/wiki/E-graph) 
saturation to explore these paths. This chapter describes that process and its 
directed-computation mode.
...
sort's relation, citing that reference. So, a theory must provide a
`@relation` bundle for the goal's sort (with a transport rule for
...
A *saturated* miss is conclusive: the search space reached a fixed point, so no
chain of enrolled rules exists. If the goal is correct, it may help to enroll 
more equations, or provide a reference to convert toward. A miss caused by
an iteration or node limit is inconclusive. In this case, it may help to raise 
those limits for one call, for example with `conversion? (iters: 32, nodes: 
20000)`, the same way `auto?` takes its parameters. The report hedges if the 
fixpoint itself was approximate. For example any `@compute` rules enrolled 
prevent the search failure from being conclusive: the fold is a strategy 
committed to one reduction order, not a closure over all of them.
